...
namespace LurkingNinja.RandomExtensions
{
    ...
    {
        public const string COLLECTION_IS_EMPTY = "The collection is empty";
        public const string MAX_MUST_BE_POSITIVE = "max must be positive";
        public const string MIN_MUST_BE_LESS_THAN_OR_EQUAL_TO_MAX = "min must be less than or equal to max";
        ...
        {
            if (span.IsEmpty) throw new ArgumentException(COLLECTION_IS_EMPTY);
        }

        public static void ThrowIfEmpty<T>(IReadOnlyList<T> list)
        {
            if (list.Count == 0) throw new ArgumentException(COLLECTION_IS_EMPTY);
        }
        ...
        {
            if (max < 0) throw new ArgumentException(MAX_MUST_BE_POSITIVE);
        }
        ...
        {
            if (min > max) throw new ArgumentException(MIN_MUST_BE_LESS_THAN_OR_EQUAL_TO_MAX);
        }
