...
```
https://github.com/LurkingNinja/com.lurking-ninja.more.git?path=Packages/com.lurking-ninja.random-extensions
```
...
## Changes
- You don't need to mock around with Nuget anymore, everything is in this package
- All credits goes to [AnnulusGames](https://github.com/AnnulusGames), please support them too!
- This library is based on [version 1.0.4](https://github.com/AnnulusGames/RandomExtensions/releases/tag/v1.0.4)

The rest of this documentation was also written by [AnnulusGames](https://github.com/AnnulusGames), I removed the installation sections since you don't need to install anything after this package.
...
```cs
using LurkingNinja.RandomExtensions;
...
Additionally, by introducing the extension package, you can use methods that support types in `System.Numerics` and Unity. For details, see the sections on [System.Numerics](#systemnumerics) and [Unity](#unity-usage).
...
Under the `LurkingNinja.RandomExtensions.Linq` namespace, there are LINQ extensions that utilize random numbers.

```cs
using System;
using System.Linq;
using LurkingNinja.RandomExtensions.Linq;
...
Under the `LurkingNinja.RandomExtensions.Collections` namespace, there are collections that hold weighted elements.
...
Under the `LurkingNinja.RandomExtensions.Algorithms` namespace, there are implementations of algorithms for generating pseudorandom numbers.
...
```cs
using LurkingNinja.RandomExtensions.Algorithms;
...
Provides extensions that support types in `System.Numerics`.
...
using LurkingNinja.RandomExtensions;
using LurkingNinja.RandomExtensions.Numerics;
...
Under the `LurkingNinja.RandomExtensions.Unity` namespace, the following extension methods are available for Unity:

```cs
using UnityEngine;
using RandomExtensions;
using LurkingNinja.RandomExtensions.Unity;
